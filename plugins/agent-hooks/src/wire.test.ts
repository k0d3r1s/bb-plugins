...
describe("claude merge", () => {
  ...
  it("is idempotent: installing twice yields the same entry count", () => {
    ...
    expect(countOurs(once)).toBe(expectedCount("claude"));
  });
...
describe("codex merge", () => {
  it("routes every hook through the shim", () => {
    ...
    expect(all.length).toBe(expectedCount("codex"));
  ...
  it("preserves gitkraken and the other codex neighbours", () => {
    const neighbours = {
      hooks: {
        pre_tool_use: [
          { hooks: [{ type: "command", command: "'/Users/x/.codex/hooks/deny-bgisolation-none.sh'" }] },
          { hooks: [{ type: "command", command: "gk ai hook run --host codex" }] },
        ],
        stop: [{ hooks: [{ type: "command", command: "npx --yes @thestackai/zclean" }] }],
      },
    };
    const out = planCodex(neighbours);
    const pre = out.hooks.pre_tool_use.flatMap((g) => g.hooks.map((h) => h.command));
    expect(pre.some((c) => c.includes("deny-bgisolation-none.sh"))).toBe(true);
    expect(pre.some((c) => c.includes("gk ai hook run"))).toBe(true);
...
describe("file safety", () => {
  ...
  it("writes atomically and leaves no temp file behind", () => {
    ...
    expect(countOurs(JSON.parse(readFileSync(f, "utf8")))).toBe(expectedCount("claude"));
