...
beforeAll(async () => {
  dataRoot = await mkdtemp(path.join(tmpdir(), "k0d3-server-"));
...
describe("resolveDataRoot", () => {
  it("resolves content/ from a source dir", () => {
    expect(resolveDataRoot("/plugins/k0d3")).toBe(path.join("/plugins/k0d3", "content"));
  });
  it("resolves content/ from a dist bundle dir (sibling of dist)", () => {
    expect(resolveDataRoot("/plugins/k0d3/dist")).toBe(path.join("/plugins/k0d3", "content"));
  });
});
...
describe("buildToolSpecs", () => {
  it("registers exactly the two tools", () => {
    ...
    expect(specs.map((s) => s.name)).toEqual(["k0d3_find_skills", "k0d3_load_skill"]);
  });
  ...
  it("find_skills gives a guidance message on zero match", async () => {
    ...
    expect(String(out)).toContain("No k0d3 skill matched");
  });
