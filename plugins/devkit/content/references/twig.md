...
Template engine for the Symfony view layer. This is the depth reference behind `Skill(devkit:php-symfony)` — load the skill first for how Twig wires into the framework; come here for the language itself.
...
`path()`/`url()` take a **route name**, never a literal path — that's how routes stay refactorable. `asset()` routes through asset-mapper so fingerprinted prod URLs Just Work. `form_*` functions render the bound Symfony form (and the CSRF field); see `Skill(devkit:php-symfony)` for the Form/Validator side.
