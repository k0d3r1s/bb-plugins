...
Complements `Skill(devkit:python-django)`. Reach here when you want a fast, type-hinted REST API on top of Django without DRF's serializer/viewset machinery — Ninja leans on Pydantic v2 and Python type hints the way FastAPI does, but mounts inside a normal Django project.
...
Ninja wins on speed-to-write and async; DRF wins when you need its ecosystem (`Skill(devkit:python-django)` → `references/drf.md`). For GraphQL use `Skill(devkit:python-strawberry-graphql)`. Ninja and DRF can coexist in one project mounted at different URL prefixes — useful when migrating off DRF incrementally.
