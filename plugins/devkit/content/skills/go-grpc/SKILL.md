...
For Go idioms and error wrapping (`errors.Is/As`, `fmt.Errorf %w`), `Skill(devkit:go-essentials)`. For concurrency patterns in interceptors and stream handlers, `Skill(devkit:go-concurrency)`. For testing gRPC services with `bufconn`, `Skill(devkit:go-testing)`.
