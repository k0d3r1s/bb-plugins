...
The CLI binary uses `anyhow::Result<T>` with `.context(...)`; the `*-core` library crate exposes a typed `thiserror` enum callers can match. `?` bridges them because anyhow absorbs any `Error`. This is the same split as `Skill(devkit:rust-essentials)` — load it for the full error model.
...
Ownership, errors, traits, Cargo workspaces: `Skill(devkit:rust-essentials)`. Async client, runtime, cancellation: `Skill(devkit:rust-async-tokio)`. The Go parallel (Cobra command tree, Viper config): `Skill(devkit:go-cobra)`.
