...
The test-first discipline (when to write the test, Red-Green-Refactor) lives in `Skill(devkit:tdd)` — this skill is mechanics, not theory. What to test where (unit vs integration vs e2e ratios, flaky-test triage): `Skill(devkit:testing-strategy)`. Booting the Symfony kernel, the DI container, fixtures, and Doctrine wiring under test: `Skill(devkit:php-symfony)`.
