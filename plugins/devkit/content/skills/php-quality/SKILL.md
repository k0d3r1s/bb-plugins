...
PHP language idioms the fixer/analyser presuppose (strict types, naming, typed properties, layout): `Skill(devkit:php-essentials)`. Turning these three gates into a real workflow — caching `vendor/`, the dependency matrix, warming the container, PR annotations: `Skill(devkit:ci-github-actions)`.
