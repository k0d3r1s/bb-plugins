...
For Python language rules (typing, layout, async): `Skill(devkit:python-essentials)`. For lint+typecheck wiring (ruff, mypy + django-stubs): `Skill(devkit:python-ruff-mypy)`. For Docker base-image choice (Debian slim, multi-stage patterns): `Skill(devkit:infra-docker-images)`. For CI integration (GitLab CI cache strategy): `Skill(devkit:ci-gitlab-ci)`.
