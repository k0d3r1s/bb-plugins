...
For FastAPI integration (request/response schemas): `Skill(devkit:python-fastapi)`. For Django/DRF (where Pydantic stays in the service layer, not the serializer): `Skill(devkit:python-django)`. For broader Python rules: `Skill(devkit:python-essentials)`. For static type checking on Pydantic models: `Skill(devkit:python-ruff-mypy)`.
