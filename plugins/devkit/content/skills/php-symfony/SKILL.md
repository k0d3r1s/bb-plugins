...
Groups gate which properties cross the wire — never expose an entity without them. For request bodies, deserialize into a DTO then validate; don't deserialize straight onto a Doctrine entity. For REST contract shape see `Skill(devkit:rest-essentials)`.
...
For persistence — entities, EntityManager, DQL, migrations, N+1: `Skill(devkit:php-doctrine)`. For the FrankenPHP worker runtime, Caddy, and worker-mode pitfalls: `Skill(devkit:infra-frankenphp)`. For PHPUnit, Symfony's `KernelTestCase`/`WebTestCase`, and DI in tests: `Skill(devkit:php-testing)`. For language-level PHP rules: `Skill(devkit:php-essentials)`. For the full Twig 3 surface: `references/twig.md`.
