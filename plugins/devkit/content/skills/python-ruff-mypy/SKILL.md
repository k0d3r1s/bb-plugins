...
For Python language idioms (typing, naming, layout): `Skill(devkit:python-essentials)`. For Django-specific lint targets and the django-stubs settings module: `Skill(devkit:python-django)`. For dep management feeding `additional_dependencies` pins: `Skill(devkit:python-uv)`. For wiring these checks into pipelines: `Skill(devkit:ci-gitlab-ci)`.
