...
Postgres 17's `MERGE`, window functions, and `LATERAL` cover 90% of "do this in pandas" reflexes. Round-tripping millions of rows over the wire to `groupby` them is always slower than the database doing it. See `Skill(devkit:postgres)`.
...
For broader Python rules: `Skill(devkit:python-essentials)`. For Django ORM patterns (when the data lives in Postgres): `Skill(devkit:python-django)`. For raw SQL optimization: `Skill(devkit:postgres)`. For generating PDFs/XLSX from these frames: `Skill(devkit:python-document-pipeline)`. For shipping data to BigQuery: `Skill(devkit:python-gcp-clients)`. For tracing slow pipelines: `Skill(devkit:observability-essentials)`.
