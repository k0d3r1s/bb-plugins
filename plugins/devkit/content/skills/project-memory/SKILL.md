...
metadata:
  ...
  related: [agent-design, using-devkit]
...
This is devkit's bundled local memory server: the official, Anthropic-maintained
...
- **Gitignored automatically.** devkit's `ensure-memory-gitignore` SessionStart hook adds
  ...
  server without devkit's hooks? Gitignore the matching host directory yourself.
- **The server does not create its parent dir.** A write to a missing state directory returns
  `ENOENT`; devkit's SessionStart hook guarantees the host-appropriate directory exists first.
- **First use needs Node + network.** `npx` fetches the package once (cached after). If Node is absent
  ...
  the rest of devkit keeps working. There are no network calls once cached.
