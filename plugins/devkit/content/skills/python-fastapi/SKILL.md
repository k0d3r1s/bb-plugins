...
For async details, type design, and packaging: `Skill(devkit:python-essentials)`. For test patterns (pytest fixtures, parametrize, async-test setup, hypothesis): `Skill(devkit:python-testing)`. For Postgres: `Skill(devkit:postgres)`.
